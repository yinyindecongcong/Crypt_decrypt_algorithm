...
用于密钥交换，其有效性建立在计算**离散对数**是很困难的基础上的。

1. **预备知识**

**本原根**：素数p的本原根是一个整数a，使得其幂模p可以产生1到p-1的所有整数，即
$$
a\ mod \ p,\ a^2\ mod \ p,\ a^3\ mod \ p\ ...\ a^{p-1}\ mod \ p各不相同
$$
对任意整数b和素数p的本原根a，可找到唯一的指数i，使得
$$
b\equiv a^i\ (mod \ p),0 \le i\le (p-1)
$$
指数i称为b的以a为底的模p离散对数，记为 $$i=dlog_{a,p}(b)$$

2. **算法流程**

素数p和本原根a公开，假设A和B希望交换密钥——

* A选择一个随机整数 $X_A < p$ ，计算 $Y_A=a^{X_A} \ mod \ p$ 
* B选择一个随机整数 $X_B < p$ ，计算 $Y_B=a^{X_B} \ mod \ p$ 
* $Y_A、Y_B$ 公开后，A选择$K = (Y_B)^{X_A} \ mod \ p$ 作为密钥，B选择$K = (Y_A)^{X_B} \ mod \ p$，两个密钥相同
